...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, T, W #x.shape [8,1024,768]=[b,spatial tokens' feature num, preset feature dims], T = time dim, W = patch's h or w
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, get_all=False, get_attn=False):
        B = x.shape[0]
        x, T, W = self.patch_embed(x) # Image to Patch Embedding
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1) # creact cls token
        x = torch.cat((cls_tokens, x), dim=1) # spatial tokens' dim + 1,[b,spatial tokens' feature num+1, preset feature dims]

        # resizing the positional embeddings in case they don't match the input at inference
        if x.size(1) != self.pos_embed.size(1):
            # print(x.shape, self.pos_embed.shape)
            pos_embed = self.pos_embed
            cls_pos_embed = pos_embed[0, 0, :].unsqueeze(0).unsqueeze(1) #pre trained cls token
            other_pos_embed = pos_embed[0, 1:, :].unsqueeze(0).transpose(1, 2) #pre trained spatial token
            P = int(other_pos_embed.size(2) ** 0.5) # P is pretrained patch number, now is 32
            H = x.size(1) // W
            ...
            x = x + self.pos_embed
        x = self.pos_drop(x) #default: no drop

        # Time Embeddings
        if self.attention_type != 'space_only':
            cls_tokens = x[:B, 0, :].unsqueeze(1)
            x = x[:, 1:]
            x = rearrange(x, '(b t) n m -> (b n) t m', b=B, t=T) # n is spatial tokens' number
